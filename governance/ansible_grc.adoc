...
+
*** _everyEvent_: When a policy is violated, the automation runs the job automation for every event.
//is there anything else that needs to be mentioned here^
...
//is there a note about _everyEvent_ that we want to add here? 
